...
const PrivacyScreen = window.Capacitor
  ? window.Capacitor.Plugins.PrivacyScreen
  : null;
...
if (!storedClientId) {
  storedClientId =
    "bc_user_" + Date.now() + "_" + Math.random().toString(16).substr(2, 8);
...
async function fetchMqttCreds() {
  ...
  try {
    const res = await fetch(
      "https://secure-room-proxy.mark-fili25.workers.dev/get-mqtt",
    );
      ...
      if (decryptedArray && Array.isArray(decryptedArray)) {
        ...
        if (localChatHistory.length > 0) {
          localStorage.setItem(
            `last_read_${pin}`,
            localChatHistory[localChatHistory.length - 1].id,
          );
        }
      } else {
        localChatHistory = [];
...
function setupPushIdentity() {
  ...
  if (window.plugins && window.plugins.OneSignal) {
    ...
    setTimeout(() => {
      ...
      if (nativeId) {
        os.User.addTag("push_id", nativeId);
        broadcastId(nativeId);
      }
    }, 1000);
    ...
    window.OneSignalDeferred.push(function (OneSignal) {
      setTimeout(() => {
        ...
        if (webId) {
          OneSignal.User.addTag("push_id", webId);
          broadcastId(webId);
        }
      }, 1000);
...
function showRoomOptions(pin, roomElement) {
  ...
  document.getElementById("options-room-title").innerText = currentName
    ? currentName
    : `Room ${displayPin}`;

  const rect = roomElement.getBoundingClientRect();
  const actionMenu = document.querySelector(
    "#room-options-overlay .action-menu",
  );
...
function renderRoomList() {
  ...
  savedRooms.forEach((pin) => {
    ...
    const lockIconHtml = isLocked
      ? `<ion-icon name="lock-closed" style="color: var(--text-muted); font-size: 1.1rem; margin-left: 6px;"></ion-icon>`
      : "";
    ...
    const unreadDotHtml = isUnread
      ? `<div class="unread-dot" style="background-color: var(--primary); width: 12px; height: 12px; border-radius: 50%; margin-left: auto; margin-right: 10px; box-shadow: 0 0 8px var(--primary); flex-shrink: 0;"></div>`
      : "";
    ...
    if (isRoomListVisible) {
      ...
      if (loginStatus) {
        loginStatus.innerText =
          "SECURITY ALERT: Passphrase must be at least 8 characters long.";
      }
...
function disconnect() {
  ...
  setTimeout(() => {
    document.getElementById("status-text").innerHTML = "Chat Deletion.";
  }, 1400);
  setTimeout(() => {
    document.getElementById("status-text").innerHTML = "Chat Deletion..";
  }, 1800);
  ...
  setTimeout(() => {
    document.getElementById("status-text").innerHTML = "Clearing Data.";
  }, 2500);
  setTimeout(() => {
    document.getElementById("status-text").innerHTML = "Clearing Data..";
  }, 2800);
  ...
  client.connect({
    ...
    onFailure: (err) => {
      ...
      if (reconnectAttempts >= 3) {
        ...
        console.log("Limite server raggiunto o rete assente. Stop tentativi.");
        alert("The server it's FULL. Please try again later");
      } else {
...
function initOneSignalNativo() {
  if (window.plugins && window.plugins.OneSignal) {
    ...
    os.Notifications.addEventListener(
      ...
      function (event) {
        const data = event.notification.additionalData;
        let incomingPin = data
          ? data.roomPin || data.fullPin || data.pin
          : null;
...
async function onMessageArrived(message) {
  ...
  if (bannedSenders.includes(data.senderId)) {
...
async function sendMessage() {
  ...
  const panelIcon = document.querySelector("#panel-bomb-icon");
  const isBombActive =
    (attachIcon && attachIcon.getAttribute("name") === "radio-button-on") ||
  ...
  if (text.toLowerCase() === "unban") {
    if (bannedSenders.length > 0) {
      const tagsToRemove = bannedSenders.map((id) => "banned_" + id);

      if (isNativeApp && window.plugins && window.plugins.OneSignal) {
        window.plugins.OneSignal.User.removeTags(tagsToRemove);
      } else if (window.OneSignalDeferred) {
        window.OneSignalDeferred.push(function (OneSignal) {
    ...
    localStorage.removeItem("bchat_banned");

    alert("All banned devices have been cleared");
    resetInputState();
    return;
  }
  ...
  const msgIdText =
    Date.now() + "-txt-" + Math.random().toString(36).substr(2, 9);
  const msgObjText = {
    id: msgIdText,
    sender: "me",
    text: text,
...
function processAndSendImage(e) {
  ...
  reader.onload = function (event) {
    ...
    img.onload = async function () {
      ...
      const msgId =
        Date.now() + "-img-" + Math.random().toString(36).substr(2, 9);
...
function handlePanelBomb(e) {
  if (e) {
    e.preventDefault();
    animateButton(e.currentTarget.querySelector("ion-icon"));
  }
  const panelIcon = document.querySelector("#panel-bomb-icon");
  const attachIcon = document.querySelector("#attach-btn ion-icon");
...
async function handleLocation(e) {
  ...
  const inviaPosizione = async (lat, lon) => {
    ...
    const msgId =
      Date.now() + "-loc-" + Math.random().toString(36).substr(2, 9);
  ...
  if (isRecording) {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        const audioType =
          mediaRecorder.mimeType || (isNativeApp ? "audio/mp4" : "audio/webm");
        ...
        reader.onloadend = async function () {
          ...
          const msgId =
            Date.now() + "-aud-" + Math.random().toString(36).substr(2, 9);

          const attachIcon = document.querySelector("#attach-btn ion-icon");
          const panelIcon = document.querySelector("#panel-bomb-icon");
          const isBombActive =
            (attachIcon &&
              attachIcon.getAttribute("name") === "radio-button-on") ||
            (panelIcon && panelIcon.getAttribute("name") === "radio-button-on");
...
if (msgInput) {
  ...
  msgInput.addEventListener("input", function () {
    ...
    const attachIcon = document.querySelector("#attach-btn ion-icon");
    const panelIcon = document.querySelector("#panel-bomb-icon");
    const isBomb = panelIcon
      ? panelIcon.getAttribute("name") === "radio-button-on"
      : false;
    ...
    if (this.value.trim().length > 0) {
      ...
      if (attachIcon.getAttribute("name") === "add") {
        attachIcon.setAttribute(
          "name",
          isBomb ? "radio-button-on" : "radio-button-off",
        );
  ...
  for (const pin of savedRooms) {
    ...
    if (incomingTopic === expectedTopic) {
      try {
        ...
        if (
          data &&
          data.senderId !== MY_CLIENT_ID &&
          !data.type.includes("WIPE")
        ) {
...
if (chatHeader) {
  chatHeader.addEventListener("click", function (e) {
    if (
      e.target.classList.contains("header-lock") ||
      e.target.closest(".header-lock")
    ) {
...
document.addEventListener("click", function (event) {
  ...
  if (panel && panel.classList.contains("open")) {
    if (
      ...
      (!attachBtn || !attachBtn.contains(event.target))
    ) {
      closeAttachmentPanel();
    }
  }
...
if (downloadViewerBtn) {
  downloadViewerBtn.addEventListener("click", async () => {
    ...
    const filename =
      nameEl && nameEl.innerText && nameEl.innerText !== "Foto"
        ? nameEl.innerText
...
if (imgGalleryInput)
  imgGalleryInput.addEventListener("change", processAndSendImage);
if (imgCameraInput)
  imgCameraInput.addEventListener("change", processAndSendImage);
...
if (docInput) {
  docInput.addEventListener("change", function (e) {
    ...
    reader.onload = async function (event) {
      ...
      const msgId =
        Date.now() + "-doc-" + Math.random().toString(36).substr(2, 9);
  ...
  if (document.visibilityState === "hidden") {
    ...
    if (client && client.isConnected()) {
      try {
        client.disconnect();
        console.log(
          "💤 App in background: Connessione MQTT tagliata per salvare slot.",
        );
      } catch (e) {}
    }
  } else if (document.visibilityState === "visible") {
    if (currentPin) {
      if (!client || !client.isConnected()) reconnect();
    } else if (savedRooms.length > 0) {
      if (!client || !client.isConnected()) connectBackgroundMQTT();
    }
  }
});
...
const btnGlobalAutoDownload = document.getElementById(
  "btn-global-autodownload",
);
const autoDownloadText = document.getElementById("autodownload-text");

function updateAutoDownloadUI() {
  if (autoDownloadText) {
    autoDownloadText.innerText = autoDownloadState
  ...
  document.addEventListener(
    ...
    (e) => {
      if (
        ...
        e.target.closest(".room-list-body")
      ) {
        return;
      }
      e.preventDefault();
...
if (chatContainerNode) {
  ...
  chatContainerNode.addEventListener("touchstart", startMsgPress, {
    passive: true,
  });
  ...
  chatContainerNode.addEventListener("contextmenu", (e) => {
    ...
    if (wrapper) {
      e.preventDefault();
      isMsgLongPress = true;
      selectedMsgIndex = Array.from(chatContainerNode.children).indexOf(
        wrapper,
      );
      showMsgOptions(wrapper);
    }
  ...
  chatContainerNode.addEventListener(
    "click",
    (e) => {
      if (isMsgLongPress) {
        e.preventDefault();
...
let bannedSenders = JSON.parse(localStorage.getItem("bchat_banned") || "[]");

document.getElementById("btn-ban-sender")?.addEventListener("click", () => {
  if (selectedMsgIndex > -1 && selectedMsgIndex < localChatHistory.length) {
    const targetMsg = localChatHistory[selectedMsgIndex];

    if (targetMsg.sender === "peer" && targetMsg.senderDeviceId) {
      if (!bannedSenders.includes(targetMsg.senderDeviceId)) {
        bannedSenders.push(targetMsg.senderDeviceId);
        localStorage.setItem("bchat_banned", JSON.stringify(bannedSenders));
      }
      if (window.plugins && window.plugins.OneSignal) {
        window.plugins.OneSignal.User.addTag(
          "banned_" + targetMsg.senderDeviceId,
          "true",
        );
      } else if (window.OneSignalDeferred) {
        window.OneSignalDeferred.push(function (OneSignal) {
          OneSignal.User.addTag("banned_" + targetMsg.senderDeviceId, "true");
        });
      }
      alert(
        "Sender banned. You will no longer receive messages from this specific device.",
      );
    } else {
...
document.getElementById("btn-report-msg")?.addEventListener("click", () => {
  if (selectedMsgIndex > -1 && selectedMsgIndex < localChatHistory.length) {
    ...
    alert(
      "Thank you. The message has been reported and permanently removed from your device.",
    );
...
if (window.Capacitor && window.Capacitor.Plugins.App) {
  ...
  App.addListener("backButton", ({ canGoBack }) => {
    ...
    const roomOptions = document.getElementById("room-options-overlay");
    if (roomOptions && roomOptions.classList.contains("active")) {
      roomOptions.classList.remove("active");
      return;
    }

    const msgOptions = document.getElementById("msg-options-overlay");
    if (msgOptions && msgOptions.classList.contains("active")) {
      msgOptions.classList.remove("active");
      selectedMsgIndex = -1;
      return;
    }

    const globalOptions = document.getElementById("global-options-overlay");
    if (globalOptions && globalOptions.classList.contains("active")) {
      globalOptions.classList.remove("active");
      return;
    }

    const newRoomOverlay = document.getElementById("new-room-overlay");
    if (newRoomOverlay && newRoomOverlay.classList.contains("active")) {
      newRoomOverlay.classList.remove("active");
      const addRoomBtn = document.getElementById("add-room-btn");
      if (addRoomBtn) addRoomBtn.style.display = "flex";
      return;
    }
